...
class ContrastTrainer(BaseTrainer):
    ...
    def _compute_loss_accuracy(logits, target, criterion):
        ...
        """
        # logits: [batch_size，1+memory bank length] ?
    ...
    def _train_moco(self, epoch, train_loader, model, model_ema, contrast,
        ...
        for idx, data in enumerate(train_loader):
            ...
            inputs = data.float().cuda(args.gpu, non_blocking=True)
            shape = inputs.shape ## bs, (n_aug+1)*6, H, W
            inputs = inputs.reshape(shape[0]*shape[1], shape[2], shape[3], shape[4])
